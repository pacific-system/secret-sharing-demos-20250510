...
本設計書では、シャミア秘密分散法を応用した「複数平文復号システム」の詳細設計を提供する。このシステムは単一の暗号化ファイルから異なるパスワードを使用して異なる平文（JSON 文書）を復号可能にするもので、「シェアトークンによる可能性の限定とパスワードによるマップ生成」という多段 MAP 方式を核心とする。
...
2. **多段 MAP 方式**：シェアトークンによる第 1 段階の絞り込みとパスワードによる第 2 段階のマッピングを組み合わせる
...
graph TD
    ...
    IA --> E
    IB --> E
    E -- パスワードAとシェアトークン A --> G[復元JSON文書A]
    E -- パスワードBとシェアトークン B --> H[復元JSON文書B]
    J --> C
    K --> C
```

この図では、復号処理は単一のプロセスであり、異なるパスワードとシェアトークンの組み合わせが入力されることで異なる文書が復元されることを示しています。実装上、復号処理は完全に同一のコードパスを通り、条件分岐なしの直線的処理で実行されます。

※注: 図では文書 A と B が同時に示されていますが、実際の処理では一度に 1 つの文書しか処理されません。どちらの文書が処理されるかは、入力されるパスワードとシェアトークンの組み合わせによって決まり、システム内部での判別処理は行われません。
...
1. **第 1 段階（シェアトークンによる限定）**：
   ...
   - ユーザーが保持するシェアトークンセットにより、全シェア空間から復号の候補となるシェアの範囲を限定
...
3. **実装方法**：
   ...
   - シェアトークン空間全体を擬似乱数生成器を用いて初期化
   - 各 ID の割り当て（A、B、未割当）は直接保存せず、パスワード A と B およびシェアトークンセットから導出
...
4. **シェアトークンの形式**：
   ...
   - 同じシェアトークンを使用すれば、同じ文書が復元される
   - セキュリティは「シェアトークン」と「パスワード」の 2 要素に依存
...
1. **第 1 段階 MAP（シェアトークン）**：
   - シェアトークンセットはユーザーに初期化時に割り当てられる
   - シェアトークンセットは単なる ID 配列

```python
def stage1_map(share_ids):
    """シェアトークンによる第1段階MAP生成"""
    # シェアトークンセットをそのまま返す（単純なフィルタとして機能）
...
def stage2_map(password, candidate_ids, salt):
    """パスワードによる第2段階MAP生成
    candidate_ids: 第1段階で限定されたシェアトークンのセット
    """
    # パスワードからキーを導出
    key = kdf(password, salt, iterations=100000, length=32)

    # キーを用いて各シェアトークンに対応するインデックスを生成
    mapping = {}
...
3. **シェアトークンの使用**：
   ...
   - シェアトークンの割り当ては初期化時にのみ行われる
   - 暗号化時には既に割り当て済みのシェアトークンを使用するだけ
   - シェアトークン + パスワード + 文書の 3 つの情報のみで暗号化が完結
...
4. **出力と永続化**：
   ...
   - マッピング情報はパスワードとシェアトークンから計算で再生成可能なため保存不要
...
| 脆弱性               | 対策                                                       |
| -------------------- | ---------------------------------------------------------- |
| シェア識別攻撃       | シェアトークン空間の混在分散配置、統計的区別不可能性の確保 |
...
2. **シェアトークン空間の隠蔽**：
   ...
   - シェアトークン空間の割り当ては外部から観測不可能
   - 統計的に区別不可能な設計

3. **多段 MAP 方式の有効性**：
   - アルゴリズムを完全に理解していても、パスワードとシェアトークンがなければ復号不可能
   - ゴミデータと実データの区別が不可能
...
シェアトークン空間の効率的かつ安全な管理のための指針：

1. **分散化と均一性**：

   - シェアトークンの分布はランダム性を維持し、パターンを形成しない
   - 任意の連続範囲において、各種別（A/B/未割当）の分布比率が一定となるよう設計
   - ブロック単位での分布検証を実施し、統計的均一性を確保

2. **効率的なルックアップ**：

   - 頻繁なシェアトークン検索に対応するため、効率的なデータ構造を採用
   - シェアトークンの検索や所属確認において条件分岐を用いない実装
   - ビット演算やルックアップテーブルを活用した定数時間アクセス

3. **安全な生成と管理**：
   - 暗号学的に安全な乱数発生器を用いたシェアトークン空間の生成
   - 初期化時にのみ ID 割り当てを行い、以後は変更しない
   - 割り当て情報は 4 つの要素（両パスワードと両シェアトークンセット）なしには再構築不可能
...
本設計書では、シャミア秘密分散法を応用した複数平文復号システムの詳細設計を提供した。核心となる「シェアトークンによる可能性の限定とパスワードによるマップ生成」という多段 MAP 方式により、単一の暗号化ファイルから異なるパスワードで異なる平文を復号可能なシステムが実現できる。
