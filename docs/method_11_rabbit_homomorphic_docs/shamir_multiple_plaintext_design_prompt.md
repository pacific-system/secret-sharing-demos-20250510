...
1. **シェア ID 空間配分**：シェア ID 空間は複数の領域に分割（A ユーザー用:30-40%、B ユーザー用:30-40%、未割当領域:20-40%）
2. **未割当領域の存在**：どのユーザーにも割り当てられない未割当領域を設け、攻撃者による統計分析を困難にする
...
7. **シェア同一性**：閾値を満たす任意のシェアの組み合わせ（例：ABC でも CDE でも）で同じ平文が復元できること

### 技術仕様

1. **暗号化対象**：UTF-8 エンコードされた JSON 文書（2 種類まで）
2. **シャミア秘密分散**：適切な閾値とシェア数の設定による秘密分散（閾値個のシェアで復号可能）
3. **シェア ID 割当**：初期化時に全シェア ID 空間の一部を各ユーザーにランダム割当し、残りを未割当として保持
...
8. **更新処理**：文書更新時は完全な再暗号化を行い、古いシェアセットは完全に破棄すること
...
5. **条件分岐のない実装**：if 文や try-catch などを使わない代替実装パターン（パスワード入力 → マップ生成 → 復号 → 結果返却の直線的処理）
6. **更新の原子性**：更新処理においてもシステム全体の一貫性を保ち、途中で中断された場合でも安全な状態を維持する方法
7. **履歴抹消**：更新時に古いシェアを確実に破棄し、過去バージョンへのアクセスを防止する技術
