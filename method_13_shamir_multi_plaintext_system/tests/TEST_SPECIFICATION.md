...
### 3.1.1 シェア閾値に関する重要な制約事項 [ID:THRESHOLD-CONSTRAINTS]

- **CONSTRAINT-3.1.1.1 [最重要]**: 本システムでは、全てのシェアが揃わないと復元できない機能しか提供しません。いわゆる (n, n) スキームとして動作し、部分的なシェア集合からの秘密復元機能は意図的に実装していません。

- **CONSTRAINT-3.1.1.2 [最重要]**: 一部のシェアだけで復号を可能にするようなバックドア、ショートカット、または弱点を発見した場合は、**直ちにテスト実行を中断し、緊急セキュリティレポート**を作成すること。緊急レポートは以下の内容を含める必要があります：
...
- **CONSTRAINT-3.1.1.3**: シェア閾値のテスト（[TEST-ID:SEC-CREATE-THRESHOLD]）では、以下を確認する必要があります：
  1. 全シェアが揃った場合のみ復元が可能であること
  2. 1 つでもシェアが欠けた場合に復元が絶対に不可能であること
  3. 数学的に部分復元が不可能であることの検証（情報理論的な安全性）
