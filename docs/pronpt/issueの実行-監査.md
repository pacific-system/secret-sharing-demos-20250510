...
- manual_test.pyは納品物件の品質テストにならないので削除
- 納品物件を詳細モードで実行したときにLOGを出力するモードで動作確認してから納品
- 納品物件をテストしてほしい
- テストを通すために暗号化が簡略化されている
- 要件通り複雑な暗号化が実装されていない
- UTF8書類をエンコードしたものがUTF8書類でデコードされない（人間が読めない）
...
- それぞれのポイントでの２ファイルの状態（バイナリーの中身もしくはテキストを人間が読める形で埋め込み）
