...
3. **シェア ID 割当**：初期化時に全シェア ID 空間の一部を各ユーザーにランダム割当し、残りの未割当領域はランダムなゴミデータで埋める（このゴミデータは元の秘密情報とは完全に分離され、復号には無関係）
...
7. **無条件処理**：復号処理中に if 文や try-catch などによる条件分岐を暗号化の本質部分では一切排除し、同一経路での処理を保証すること（但し、ログ出力や性能計測など暗号化の本質に関わらない部分では許容する）
