...
オプション:
  --size, -s       : 生成するパーティションマップキーの総数 (デフォルト: 100)
  --ratio, -r      : A:B:未割当の比率 (デフォルト: "35:35:30")
  --output-a       : A用パーティションマップキーの出力ファイル名 (デフォルト: shares-a-{uuid}.bin)
  --output-b       : B用パーティションマップキーの出力ファイル名 (デフォルト: shares-b-{uuid}.bin)
...
- A 用と B 用のパーティションマップキーを別々の高度に暗号化されたバイナリファイルに保存
- AB を俯瞰できるマスターデータは出力しない
- 各ファイルに一意の UUID を付与して重複を防止

#### 2.2. パーティションマップキー生成コマンド

```
shamir-multi-crypt generate --size <シェア数> --output <ファイル名>

オプション:
  --size, -s      : 生成するパーティションマップキーの数 (デフォルト: 100)
...
```
shamir-multi-crypt encrypt --file <JSONファイル> --token <パーティションマップキー> --output <暗号化ファイル> --type <タイプ>

オプション:
  --file, -f      : 暗号化するJSON文書ファイルパス
  --token, -t     : 文書用のパーティションマップキー
  ...
  --token-password: パーティションマップキーファイルのパスワード（指定しない場合はプロンプト）
```
...
- 単一の JSON 文書を読み込み
- 暗号化されたパーティションマップキーファイルを復号して読み込み
...
```
shamir-multi-crypt decrypt --input <暗号化ファイル> --token <パーティションマップキー> --output <出力JSONファイル>

オプション:
  --input, -i     : 暗号化ファイルパス
  --token, -t     : パーティションマップキー
...
- 暗号化バイナリファイルとパーティションマップキーファイルを読み込み
- パスワードをプロンプト（指定されていない場合）
...
```
shamir-multi-crypt update --input <暗号化ファイル> --file <新JSONファイル> --token <パーティションマップキー> --output <更新後ファイル>
...
- 元の暗号化ファイル、新しい JSON 文書、パーティションマップキーファイルを読み込み
- パスワードをプロンプト（指定されていない場合）
...
def load_share_token(token_file, password=None):
    """
    暗号化されたパーティションマップキーファイルを読み込み

    Args:
        token_file: パーティションマップキーファイルパス
        password: パスワード（Noneの場合はプロンプト）

    Returns:
        tuple: (パーティションマップキー, ファイルUUID)
    """
    # ファイル読み込み
    # パスワードが必要ならプロンプト
    # 復号と解析
    # パーティションマップキーとUUIDを返却
```
...
def initialize_command(args):
    ...
    # パーティションマップキー空間の生成
    # A/B/未割当シェアに分割
    # A用パーティションマップキーをUUID付きで暗号化して保存
    # B用パーティションマップキーをUUID付きで暗号化して保存
    # 成功メッセージの表示
```
...
initialize_parser = subparsers.add_parser(
    'initialize',
    help='システムを初期化し、パーティションマップキー空間を分割',
    description='システムを初期化し、パーティションマップキー空間をA用、B用、未割当に分割します。'
             'A用とB用のパーティションマップキーは別々の暗号化ファイルに保存され、マスターデータは出力されません。'
...
encrypt_parser = subparsers.add_parser(
    ...
    description='一つのJSON文書とパーティションマップキーを用いて暗号化ファイルを生成します。'
             '生成されたファイルは該当するパスワードとパーティションマップキーで復号可能です。'
