...
1. **パーティション定義と整数値による明示化**：
   ...
   - ユーザーパーティション設計を`constants.py`で整数値として明示的に定義
     ```
     # ユーザーパーティション設計（整数値で明示的に定義）
   ...
   - シェア ID 空間のサイズは自動計算される
     ...
     ```
   - ユーザー A/B それぞれに対して常に**完全に同一**のパーティションサイズを割り当て
...
4. **有効シェア数と暗号化容量の関係**：
   ...
   - 各ユーザーが実際に使用できるのは`USER_ACTIVE_SHARES`の数（例：2000 シェア）
   - 最大データ容量 = `USER_ACTIVE_SHARES × CHUNK_SIZE`（例：2000 × 64 バイト = 128KB の実データ容量）
...
5. **パーティション空間の固定性と設定**：
   ...
   - ユーザーパーティション設計は `method_13_shamir_multi_plaintext_system/shamir/constants.py` で以下のように定義:
