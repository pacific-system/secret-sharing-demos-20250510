...
- 領域分布 = `partition_distribution`：シンプルな整数配列。パーティションマップキーをパスワードで復号すると領域分布が得られる。この配列にはパーティションに属するシェア ID（インデックス）が含まれる。
- パーティションマップキー = `partition_map_key`：領域分布をパスワードで堅牢に暗号化したものです。
...
- **入力**:
  - A 領域用パスワード（第 2 段階 MAP でも使用）
  - B 領域用パスワード（第 2 段階 MAP でも使用）
  - 任意: システムパラメータ
- **処理**:
  ...
  4. A 領域と B 領域に対応するパーティションマップキーを生成
     - A 用パーティションマップキーの生成には A 領域用パスワードを使用
     - B 用パーティションマップキーの生成には B 領域用パスワードを使用
     - パーティションマップキーは暗号化された形で領域分布情報を含む
     - 各パスワードは後の第 2 段階 MAP 生成でも使用される
- **出力**:
...
- **特性**:
  ...
  - パスワード保護：パーティションマップキーはパスワードで暗号化されるため、キーが漏洩してもパスワードなしではアクセス不可
  - パスワード一貫性：生成時に使用したパスワードは、その後の更新処理や読取処理でも同じパスワードを使用する必要がある
  - 暗号化保護：パスワードを使用した認証付き暗号化（AES-GCM 等）により、パーティションマップキーの改ざんを検知可能
...
- **処理**:
  ...
  2. パーティションマップキーから第 1 段階 MAP を再生成
     - パーティションマップキーをパスワードで復号し、対応する領域の ID 分布を再構築
     - 認証付き暗号化の検証によりパーティションマップキーの完全性を確認
