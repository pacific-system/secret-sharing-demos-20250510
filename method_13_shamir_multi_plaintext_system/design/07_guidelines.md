...
5. **パスワード使用**: パーティションマップキーの暗号化には全文そのままのパスワードが使用され、第 2 段階 MAP 生成には処理（ハッシュ化）されたパスワードが使用される
...
2. **パーティションマップキーの復号**:
   ...
   ```python
   # パーティションマップキーを全文そのままのパスワードで復号する
...
2. **鍵管理**:
   ...
   - パーティションマップキーは全文そのままのパスワードで暗号化
   - 第 2 段階 MAP の生成には処理されたパスワードを使用
