...
class State:
    ...
    def get_neighbor_states(self) -> List[int]:
        """
        隣接状態のリストを取得
    ...
    def __repr__(self):
        ...
        transitions_str = ", ".join(
            f"{s}:{p:.2f}" for s, p in self.transitions.items()
        )
        return f"State({self.state_id}, transitions=[{transitions_str}])"
...
def get_biased_random_generator(bias: float, noise: float = 0.1) -> Callable[[], float]:
    def generator():
        raw = secrets.randbelow(10000) / 10000.0

        if bias <= 0.5:
            biased = raw ** (1.0 + (0.5 - bias) * 4)
        else:
            biased = 1.0 - ((1.0 - raw) ** (1.0 + (bias - 0.5) * 4))

        if noise > 0:
            noise_value = (secrets.randbelow(10000) / 10000.0 - 0.5) * noise * 2
            biased = max(0.0, min(1.0, biased + noise_value))
...
def test_state_matrix():
    ...
    # バイアス乱数のテスト
    print("\nバイアス乱数のテスト:")
    biased_gen = get_biased_random_generator(0.7)
