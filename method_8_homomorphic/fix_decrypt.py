...
def remove_homomorphic_mask(data: bytes, key_data: Dict[str, Any]) -> bytes:
    ...
    # マスクの除去 - 複数の方法を試す
    result1 = bytearray(len(data))  # XOR方式
    result2 = bytearray(len(data))  # 減算方式
    result3 = bytearray(len(data))  # 加算方式

    for i in range(len(data)):
        result1[i] = (data[i] ^ mask[i % len(mask)]) % 256  # XOR方式
        result2[i] = (data[i] - mask[i % len(mask)] + 256) % 256  # 減算方式
        result3[i] = (data[i] + mask[i % len(mask)]) % 256  # 加算方式

    # 各方式の結果をテスト
    for result, method in [(bytes(result1), "XOR"), (bytes(result2), "減算"), (bytes(result3), "加算")]:
        try:
            # UTF-8としてデコードを試みる
            text = result[:100].decode('utf-8', errors='strict')
            print(f"{method}方式で正しくデコードされました。この方式を採用します。")
            return result
        except UnicodeDecodeError:
            # Base64としてデコードを試みる
            try:
                text = safe_base64_decode(result[:100]).decode('utf-8', errors='strict')
                print(f"{method}方式でBase64後に正しくデコードされました。この方式を採用します。")
                return result
            except:
                pass

    # いずれの方法でもデコードできない場合、XOR方式を返す
    print("どの方式でも明確に正しいデコードができませんでした。XOR方式を採用します。")
